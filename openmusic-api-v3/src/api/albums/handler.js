...
class AlbumsHandler {
  ...
  async postUploadCoverHandler(request, h) {
    ...
    if (!cover || !cover.hapi) {
      throw new InvariantError(
        "No file uploaded or invalid payload structure."
      );
    }
    ...
    if (!allowedMimeTypes.includes(cover.hapi.headers["content-type"])) {
      throw new InvariantError(
        "File type not allowed. Only images are permitted."
      );
    }
  ...
  async getAlbumLikesHandler(request, h) {
    const { id: albumId } = request.params;
    const { count, fromCache } = await this._service.getAlbumLikesCount(
      albumId
    );
